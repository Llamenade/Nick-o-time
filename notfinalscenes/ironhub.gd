...
func _process(delta):
	if abs(rotation_x - 90) <= 15 and abs(rotation_z - 0) <= 15 and abs(rotation_y - 66) <= 15:
		Global.is_charbydis_level = true
	else:
		Global.is_charbydis_level = false
	if abs(rotation_x - 133) <= 15 and abs(rotation_z - 0) <= 15 and abs(rotation_y - 30) <= 15:
		Global.is_achilles_level = true
	else:
		Global.is_achilles_level = false
	if abs(rotation_x - 120) <= 15 and abs(rotation_z - 0) <= 15 and abs(rotation_y - 0) <= 15:
		Global.is_manticore_level = true
	else:
